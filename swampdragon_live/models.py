...
def post_save_handler(sender, instance, created, **kwargs):
    if ContentType.objects.exists():
        ...
        if created:
            queryset_hash = hashlib.sha1(str('%d:qs' % instance_type.pk).encode('utf-8')).hexdigest()
            pushers = push_new_content_for_queryset(channel_cache, queryset_hash, instance.pk)
        else:
            instance_hash = hashlib.sha1(str('%d:%d' % (instance_type.pk, instance.pk)).encode('utf-8')).hexdigest()
